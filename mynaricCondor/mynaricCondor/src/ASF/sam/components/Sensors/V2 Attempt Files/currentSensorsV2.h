...
#define LIST_OF_CURRENT_CHANNELS	X(  eAdcInternal28VPwrCurrentMonitorCH1	    ,0.69                    ,0 )\
                                    X(  eAdcInternal28VPwrCurrentMonitorCH2	    ,0.69                    ,0 )\
									X(  eAdcInternal28VPwrVoltageMonitorCH1	    ,0.0017                  ,0 )\
									X(  eAdcInternal28VPwrVoltageMonitorCH2	    ,0.0017                  ,0 )\
									X(  eAdcInternal7VPwrVoltageMonitorCH1	    ,0.00018                 ,0 )\
									X(  eAdcInternal7VPwrVoltageMonitorCH2	    ,0.00018                 ,0 )\
									X(  eAdcInternal3_3VPower_VoltageMonitor    ,0.00091                 ,0 )\
									X(  eAdcInternal3VPower_VoltageMonitor	    ,0.00091                 ,0 )\
									X(  eAdcInternalPowerCurrentMonitor			,0.69                    ,0 )\
									X(  eAdcInternalCanPwrCurrentMonitorCH1	    ,0.69   		         ,0 )\
									X(  eAdcInternalCanPwrCurrentMonitorCH2	    ,0.69	                 ,0 )\
									X(  eAdcExLnaSingleModeldCurrentMonitor	    ,0.000220                ,0 )\
									X(	eAdcExHpaSingleModeld_CurrentMonitor	,0.000220     			 ,0 )\
									X(	eAdcExHpaMultiModeld_CurrentMonitor1	,0.0026               	 ,-0.9818 )\
									X(  eAdcExternalLnaSm_ld_Bfm                ,0.000754           	 ,0 )\
									X(	eAdcExternalHpaSm_ld_Bfm	  		    ,0.000754				 ,0 )\
									X(	eAdcExHpaMultiModeld_CurrentMonitor2	,0.0027               	 ,-1.1719 )
/* Exported macro ----------------------------------------------------------- */
...
/**
* @brief	    Return value of current in mA and raw value
*/
eMpbError_t eCurrentSensingGetCurrentValue(eCurrentChannelsScaling_t eChannel, float *fCurrentmA, uint16_t *ulCurrentRaw);
